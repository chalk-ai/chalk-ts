...
import { Code, codeFromJSON, codeToJSON } from "../../../google/rpc/code.pb";
import { Agent } from "../../auth/v1/agent.pb";
...
function createBaseGetAuditLogsRequest(): GetAuditLogsRequest {
  return { startTime: undefined, endTime: undefined, endpointFilter: [] };
}
...
export const GetAuditLogsRequest: MessageFns<GetAuditLogsRequest> = {
  encode(message: GetAuditLogsRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    for (const v of message.endpointFilter) {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3: {
          ...
          message.endpointFilter.push(reader.string());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): GetAuditLogsRequest {
    return {
      ...
      endpointFilter: globalThis.Array.isArray(object?.endpointFilter)
        ? object.endpointFilter.map((e: any) => globalThis.String(e))
        : [],
    };
  },
  ...
  toJSON(message: GetAuditLogsRequest): unknown {
    ...
    if (message.endpointFilter?.length) {
